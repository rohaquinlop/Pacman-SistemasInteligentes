...
class CornersProblem(search.SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        "*** YOUR CODE HERE ***"
        # Compute the distance from each corner to all the positions on the board

        corner1_1 = [[99999 for _ in range(
            self.walls.height)] for _ in range(self.walls.width)]
        corner1_top = [[99999 for _ in range(
            self.walls.height)] for _ in range(self.walls.width)]
        corner_right_1 = [[99999 for _ in range(
            self.walls.height)] for _ in range(self.walls.width)]
        corner_right_top = [[99999 for _ in range(
            self.walls.height)] for _ in range(self.walls.width)]
        ...
        while not queue.isEmpty():
            ...
            successors = self.getSuccessors(pos)

            for successor in successors:
                x, y = successor[0][0]

                if corner1_1[x][y] == 99999:
                    corner1_1[x][y] = dst + 1
        ...
        # Corner right,1
        queue = util.Queue()
        queue.push((((right, 1), []), 0))
        corner_right_1[right][1] = 0
        while not queue.isEmpty():
            pos, dst = queue.pop()
        ...
        # Corner right,top
        queue = util.Queue()
        queue.push((((right, top), []), 0))
        corner_right_top[right][top] = 0
        while not queue.isEmpty():
            pos, dst = queue.pop()
    ...
    def getStartState(self):
        ...
        startState = (self.startingPosition, corners)
    ...
    def getSuccessors(self, state):
        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            leftCorners = list(state[1])
            if (nextx, nexty) in leftCorners:
                leftCorners.remove((nextx, nexty))

            nextState = ((nextx, nexty), tuple(leftCorners))
...
def foodHeuristic(state, problem):
    ...
    for food in foodPos:
        ans = max(ans, util.manhattanDistance(position, food))
